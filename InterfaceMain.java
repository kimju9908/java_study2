...
/*
인터페이스: 자바에서 다중상속을 지원하기 위해서 만들어짐(자바는 다중상속 지원 x)
개발에 있어서 설게명세서 개념이라고 생각하면 됨
완벽한 추상화 : 모든 메서드가 추상메서드 임 (단 예외가 있음)
인터페이스에 포함되는 필드: final static 이 자동으로 추가 됨(컴파일러) - 상수가 됨
인터페이스에 포함되는 메서드: public abstact가 자동으로 추가 됨(컴파일러) - 추상메서드
정적메서드 : 객체와 상관없음으로 사용해도 됨(구현부가 있음)
인터페이스 상속은 implements 키워드를 사용하고, 상속받는 갯수는 무한대
